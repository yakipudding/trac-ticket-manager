...
DROP PROCEDURE IF EXISTS [dbo].[RegisterTracTicket]
GO
	...
	BEGIN
	   ...
	   BEGIN
		   ...
		   VALUES
				...
				,@Complete
				,@Reporter
				,CASE WHEN CHARINDEX(@Owner, 'admin') <> 0 AND @Status <> 'closed' THEN CONVERT(date, GETDATE()) ELSE NULL END --★
				)
	   END
	   ...
	   BEGIN
			...
			SET
			  ...
			  ,[Complete] = @Complete
			  ,[Reporter] = @Reporter
		    ,[LastOwnerDate] = CASE WHEN CHARINDEX(@Owner, 'admin') <> 0 AND @Status <> 'closed' THEN CONVERT(date, GETDATE()) ELSE NULL END --★
